...
gf_init(void) {
    ...
    if (majorNumber < 0) {
        printk(KERN_ALERT
        "GF failed to register\n");
        return majorNumber;
    }
    printk(KERN_INFO
    "Kernel: registered - major number %d\n", majorNumber);
    ...
    if (IS_ERR(gfClass)) {
        ...
        printk(KERN_ALERT
        "Failed to register gf device class\n");
        return PTR_ERR(gfClass);
    }
    printk(KERN_INFO
    "gf device class registered correctly\n");
    ...
    if (IS_ERR(gfDevice)) {
        ...
        unregister_chrdev(majorNumber, DEVICE_NAME);
        printk(KERN_ALERT
        "Failed to create gf device\n");
        return PTR_ERR(gfDevice);
    }
    printk(KERN_INFO
    "Kernel: gf device class created correctly\n");
    return 0;
}
...
static int dev_open(struct inode *inodep, struct file *filep) {
    printk(KERN_INFO
    "Kernel: device connect successful \n");
    return 0;
}
...
static int dev_release(struct inode *inodep, struct file *filep) {
    printk(KERN_INFO
    "Kernel: device disconnect successful\n");
    return 0;
}
