...
#define SIZE_CLASS_bin_yes(block_size, pages) \
        { block_size, kSuperBlockSize, kSuperBlockSize / (block_size), kSuperBlockSize / (block_size) },
...
namespace crpm {
    SizeClass::SizeClass() :
            size_classes{{0, 0, 0, 0}, SIZE_CLASSES},
            size_class_lookup{0} {
        size_t size_idx = 0;
        for (size_t entry_idx = 1; entry_idx < kMaxSizeClasses; ++entry_idx) {
            SizeClassEntry *entry = &size_classes[entry_idx];
            size_t block_size = entry->block_size;
            while (size_idx <= block_size) {
                size_class_lookup[size_idx] = entry_idx;
                ++size_idx;
            }
        }
    }
