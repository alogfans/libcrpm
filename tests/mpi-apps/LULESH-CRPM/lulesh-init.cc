   ...
   m_refdens(Real_t(1.0))
{
   ...
   m_regNumList = (Index_t *) crpm_malloc(g_pool->pool, sizeof(Index_t) * numElem());  // material indexset
...
Domain::SetupThreadSupportStructures()
{
  ...
  if (numthreads > 1) {
    ...
    Index_t *nodeElemCount = (Index_t *) crpm_malloc(g_pool->pool, sizeof(Index_t) * numNode());
    ...
    m_nodeElemStart = (Index_t *) crpm_malloc(g_pool->pool, sizeof(Index_t) * (numNode() + 1));
    ...
    m_nodeElemCornerList = (Index_t *) crpm_malloc(g_pool->pool, sizeof(Index_t) * m_nodeElemStart[numNode()]);
...
Domain::SetupCommBuffers(Int_t edgeNodes)
{
  ...
  this->commDataSend = (Real_t *) crpm_malloc(g_pool->pool, sizeof(Real_t) * comBufSize);
  this->commDataRecv = (Real_t *) crpm_malloc(g_pool->pool, sizeof(Real_t) * comBufSize);
...
Domain::CreateRegionIndexSets(Int_t nr, Int_t balance)
{
...
#endif
   this->numReg() = nr;
   m_regElemSize = (Index_t *) crpm_malloc(g_pool->pool, sizeof(Index_t) * numReg());
   m_regElemlist = (Index_t **) crpm_malloc(g_pool->pool, sizeof(Index_t *) * numReg());
   Index_t nextIndex = 0;
   ...
   //If we have more than one region distribute the elements.
      ...
      Int_t* regBinEnd = (Int_t *) crpm_malloc(g_pool->pool, sizeof(Int_t) * numReg());
   ...
   for (Index_t i=0 ; i<numReg() ; ++i) {
      m_regElemlist[i] = (Index_t *) crpm_malloc(g_pool->pool, sizeof(Index_t) * regElemSize(i));
      regElemSize(i) = 0;
   }
