...
int main(int argc, char** argv)
{
   ...
   sim = (SimFlat *) crpm_get_root(g_pool->pool, 0); 
   validate = (Validate *) crpm_get_root(g_pool->pool, 1); 
   if (!sim) {
      sim = initSimulation(cmd);
      printSimulationDataYaml(yamlFile, sim);
      printSimulationDataYaml(screenOut, sim);
      crpm_set_root(g_pool->pool, 0, sim);

      validate = initValidate(sim); // atom counts, energy
      crpm_set_root(g_pool->pool, 1, validate);
   }
   ...
   crpm_protect(g_pool, 2, &iStep, sizeof(int));
   crpm_protect(g_pool, 3, &isEam, sizeof(int));
   ...
   for (; iStep<nSteps;)
   {
      ...
      if (cmd.cp_stride > 0 && (iStep % cmd.cp_stride == 0)) { 
